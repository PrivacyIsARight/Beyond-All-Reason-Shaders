...
void main() {
    ...
    vec2 fuv = fragCoord/u_resolution.xy;
    float aspect = u_resolution.x/u_resolution.y;
    fuv.x *= aspect;

    vec3 col = getPaintedSkyAndMountains(fragCoord, u_resolution.xy, u_time);

    col = getPaintedWaterAndSand(fragCoord, u_resolution.xy, u_time, col);

    float landDist = getLand(fuv, aspect);
    float landMask = smoothstep(0.003, 0.0, landDist);
    vec3 cRock = vec3(0.08, 0.1, 0.12);
    col = mix(col, cRock, landMask);

    vec4 trees = getPaintedTrees(fuv, u_resolution.xy, u_time, aspect);
    col = mix(col, trees.rgb, trees.a);

    vec4 grass = getIslandGrass(fuv, u_resolution.xy, u_time, aspect);
